		...
		for (int i = 1; i <= vagueDegree; i++) {
			...
			for (int j = 0; j < height - 3; j++) {
				for (int k = 0; k < width * 3; k++) {
					...
					sumColor = sumColor + pixelsRawSource[currentPixel + 3]; // �?�一点
					sumColor = sumColor
							+ pixelsRawSource[currentPixel + width * 3]; // 下一点
					pixelsRawNew[currentPixel] = Math.round(sumColor / 4); // 设置�?素点
				}
			}

			for (int j = 0; j < pixels.length; j++) {
				pixels[j] = Color.rgb(pixelsRawNew[j * 3 + 0],
						pixelsRawNew[j * 3 + 1], pixelsRawNew[j * 3 + 2]);
			}
		}
